        ...
        for (let i = this.foods.length - 1; i >= 0; i--) {
            ...
            if (head.x === food.x && head.y === food.y) {
                ...
                if (food.data.speedMod !== 0) {
                    // Decrease interval = faster speed
                    // speedMod 2 means faster, so subtract from interval
                    // speedMod -2 means slower, so add to interval
                    // Python logic: speed += mod. Higher speed value = faster.
                    // Here: Lower interval = faster.
                    // Let's approximate: 1 speed unit ~ 2ms interval change?
